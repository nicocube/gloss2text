...
module.exports = (function () {
    ...
    if (a.length === 0) {
      ...
      for (; i < l ; i += 1, c = a[i]) {
        ...
        if (i < l-1 && p instanceof Array) {
          r.chain(p.map(this.parse.bind(this)))
          p = this.definition
        }
      }
      ...
      if (typeof p === 'string') r.chain(split_clean_str(p).map(this.parse.bind(this)))
      else if (p instanceof Array) r.chain(p.map(this.parse.bind(this)))
      return r
    }
  }
  Pattern.prototype.parse = function(p) {
    if (this.pattern.indexOf(p) !== -1) {
      ...
      var r = this.tr.parseRule(p)
        , k = p.replace(/^(.*)>.*$/,'$1')
      return new Selector(this, k !== p ? k : undefined, r)
    }
  }
  ...
  MorphChain.prototype.apply = function(f) {
    ...
    else if (typeof this.f !== 'undefined' && this.f in f) p = f[this.f]
    else p = Object.keys(f).map(function(k) { return f[k] })[0]

    var i = 0
      , l = this._.length
      , c = this._[0]
    for (; i < l ; i += 1, c = this._[i]) {
      var j = 0
        , m = c.length
        , s = c[0]
      for (; j < m ; j += 1, s = c[j]) {
  ...
  function Selector(p, f, r) {
    this.p = p
    this.f = f
    this.r = r
  }
  Selector.prototype.apply = function(r) {
    if (this.r) {
